...
class Trajectory(object):
    ...
    ########## 尝试改为画0到当前time_ms所有的点在图片上
    def display_on_image_02time_ms(self, time_ms, image, cam_model, only_complete = False, color_text = (0, 0, 255), no_label = False, custom_text = None, complete_marker = False, velocity_label = False):
        det_track = None;
                ...
                for i in range(0, time_ms + 1):
                    ...
                    if traj_point: # 若该时刻轨迹点存在
                        ...
                        if (time_ms == i):# 仅给当前时刻加3Dbox
                            ...
                            pt_pix = cam_model.project_points(np.array([(traj_point.x, traj_point.y, 0.0)]));
                            pt_pix = (int(pt_pix[0]), int(pt_pix[1]));
                            image = cv2.circle(image, pt_pix, 2, self.get_color(), 1);
