...
class DetZoneFNED(object):
    ...
    def in_zone(self, pt_FNED):
        ...
        if self.pt_det_zone_FNED is not None:
            ...
            # 读源码解决问题 https://answers.opencv.org/question/206858/pointpolygontest-throws-error-for-some-reason/
            # https://docs.opencv.org/3.4.4/dc/d48/tutorial_point_polygon_test.html
            # 用int()对pt点数据强制转换
            pt = (int(pt_FNED[0]), int(pt_FNED[1]));
            # 用astype(int),对Counter强制类型转换
            res = cv2.pointPolygonTest(contour.astype(int), pt, False)
            if (res < 1):
                in_zone = False;
