    ...
    if display_traj:
        for traj in traj_list:
            ...
            if not (traj.get_id() in traj_ignore_list_1) and not (traj.get_id() in traj_ignore_list_2) and not (traj.get_id() in traj_ignore_list_3):
                ...
                #########3# 尝试改为画0到当前time_ms所有的点在图片上
                # 要实现速度标签打上去
                image_current, det_track = traj.display_on_image_02time_ms(time_ms, image_current, cam_model, only_complete = only_complete, complete_marker = complete_marker, no_label = no_label, velocity_label = velocity_label);
                if not det_track is None:
                    det_track_list.append(det_track);

    return image_current, det_track_list;

def display_conflict_traj_on_image(time_ms, cam_model, image, traj_list, traj_ignore_list_1 = [], traj_ignore_list_2 = [], traj_ignore_list_3 = [], det_zone_FNED_list = [], no_label = False, display_traj = True, only_complete = False, complete_marker = False, velocity_label = False):
    """Display the trajectory on an image
